	...
	public static void main(String[] args) {
        try {
			...
			for (int i = 2; i <= 14; i++) {
				...
				for (int j = 1; j <= 4; j++) {
					switch (j) {
					case 1: color = '♣'; table.addCell(new Paragraph(card + color + " " + black));
						break;
					case 2: color = '♦'; table.addCell(new Paragraph(card + color + " " + red));
						break;
					case 3: color = '♥'; table.addCell(new Paragraph(card + color + " " + red));
						break;
					case 4: color = '♠'; table.addCell(new Paragraph(card + color + " " + black));
						break;
					}
				}
			}
